    ...
    <div className={styles.modal}>
      <div className={styles.modal_content}>
        ...
        <div className={styles.tabs}>
          ...
          <div className={`${styles.tab} ${activeTab === 1 ? styles.active : ''}`}>
            <div className={`${styles.circle} ${activeTab >= 1 ? styles.active_circle : ''}`}>
            <span><Money fill={`${activeTab >= 1 ? '#FFFFFF' : ''}`}/></span>
              <span className={`${styles.tab_title} ${activeTab >= 1 ? styles.active_title : ''}`}>Pago</span>
            </div>
            <div className={`${styles.line} ${activeTab > 1 ? styles.active_line : ''}`} />
          </div>
          <div className={`${styles.tab} ${activeTab >= 2 ? styles.active : ''}`}>
            <div className={`${styles.circle} ${activeTab >= 2 ? styles.active_circle : ''}`}>
              <span className={styles.tab_icon}><Trophy fill={`${activeTab >= 2 ? '#FFFFFF' : ''}`}/></span>
              <span className={`${styles.tab_title} ${activeTab >= 2 ? styles.active_title : ''}`}>Confirmado</span>
            </div>
          </div>
        </div>
        <div className={styles.tab_content}>
          {/* {activeTab === 1 && <TabSelectChallenge />} */}
          {activeTab === 1 && <TabSelectPayment/>}
          {activeTab === 2 && <TabConfirmPayment handleNext={handleNext}/>}
          {activeTab === 3 && 
        ...
        <div className={styles.footer}>
          ...
          {activeTab === 1 && (
            <>
              ...
              <button onClick={handleNext} className={styles.modal_button}>Ya hice la transferencia</button>
            </>
          )}
          {activeTab === 2 && (
            <>
